	    ...
	    touch: function touch($sprite, $e) {
	        ...
	        if (!$sprite.$scroll.touching) {
	            ...
	            if (Math.abs(deltaX) >= 1 && deltaTime > 1) {
	                var newSpeedX = ($e.canvasX - $sprite.$scroll.startPos.x) * 6 / devicePixelRatio;
	            ...
	            if (Math.abs(deltaY) >= 1 && deltaTime > 1) {
	                var newSpeedY = ($e.canvasY - $sprite.$scroll.startPos.y) * 6 / devicePixelRatio;
	    ...
	    option.scroll = _extends({
	        ...
	        maxScrollX: function maxScrollX() {
	            ...
	            this.getChildren().forEach(function (child) {
	                var currentMax = child.getSelfStyle('tx') + child.getOuterRect().tw - _this.getStyle('tw');
	        ...
	        maxScrollY: function maxScrollY() {
	            ...
	            var max = 0;
	            this.getChildren().forEach(function (child) {
	                var currentMax = child.getSelfStyle('ty') + child.getOuterRect().th - _this2.getStyle('th');
	                if (currentMax > max) max = currentMax;
	            });
	            return max;
	        },
