export default `
    <article id="Transition-API">
        <h1>Transition API</h1>

        <p>Transition用于为Sprite设置按一定规律变化的属性，其它原理是创建一个与时间有关的function，每次调用时将根据当前帧的渲染时间计算出当前的值。</p>

        <p class="tip">Tips：这里根据“当前帧的渲染时间”来计算，而不是用“当前时间”来计算，是为了避免当js运行时间超过1毫秒时，多个Transition的参考时间不同而出现的微小误差。</p>

        <p>例如下面这个Transition，不断调用，将输出从100到</p>
