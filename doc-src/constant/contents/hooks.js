module.exports = `
    <article id="钩子">
        ...
        <p>钩子包括easycanvas的一些状态钩子和自定义钩子，用于在每一动画帧的各个阶段进行数据的编辑，以及处理多个sprit之间的互相影响。</p>
        
        <h2>状态钩子</h2>

        <p>Easycanvas目前有ticked、beforeTick、removed、beforeRemove四个钩子。前两者在每次渲染（即每帧）时调用，后两者在sprite的remove方法触发时调用。beforeTick也可以用于一些比较复杂的动画的每帧坐标计算：</p>
